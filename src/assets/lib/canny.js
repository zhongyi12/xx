...
module.exports = (function() {
  ...
  GrayImageData = (function() {
    ...
    GrayImageData.prototype.drawOn = function(canvas) {
      ...
      for (i = _i = 0, _len = _ref.length; _i < _len-1; i = _i, _i+=4) {
        color = _ref[i];
        imgData.data[i] = 255-color;
        imgData.data[i+1] = 255-color;
        imgData.data[i+2] = 255-color;
  ...
  CannyJS.sobel = function(imgData) {
    ...
    xFiler = [[-1, 0, 1], [-2, 0, 2], [-1, 0, 1]];
    yFiler = [[1, 2, 1], [0, 0, 0], [-1, -2, -1]];
    copy = imgData.copy();
    copy.fill(0);
  ...
  CannyJS.nonMaximumSuppression = function(imgData) {
      ...
      //refer: https://en.wikipedia.org/wiki/Canny_edge_detector
    var copy;
    copy = imgData.copy();
    copy.fill(0);
    var diff=1
    imgData.eachPixel(3, function(x, y, c, n) {
        // better edge thinning method but low efficiency
        // copy.data[x][y] = 0;
        // if( (n[0][1]-n[2][1])<diff && Math.abs(n[1][0]-n[1][2])>diff ){
        //     //north–south direction
        //     if (n[1][1] > n[1][0] && n[1][1] > n[1][2]) {
        //       copy.data[x][y] = n[1][1];
        //     }
        // }
        // else if( (n[1][0]-n[1][2])<diff && Math.abs(n[0][1]-n[2][1])>diff ){
        //     //east–west direction
        ...
        // }
          ...
          if(x<1 || y <1 || x> imgData.width-2 || y>imgData.height-2){
              copy.data[x][y] = 0; //remove png image black border
          }
          else{
              if (n[1][1] > n[0][1] && n[1][1] > n[2][1]) {
                  copy.data[x][y] = n[1][1];
                ...
                else if(n[1][1] > n[0][0] && n[1][1] > n[2][2]){
                  copy.data[x][y] = n[1][1];
                }
                else {
                  copy.data[x][y] = 0;
