<template lang="html">
  <div class="root-wrapper">
    ...
    <!-- <div v-if="model.localTeach.curPoint.length === 1 && isShowPointEdit === true"
      ...
      <span style="color:rgb(132, 132, 132);height:30px;line-height:30px;font-size: 13px;">
        {{ $t('remain') }}:
      </span>
    ...
    <div :class="{'running-disable':model.localTeach.isTeachRunningUUID !== '' }"
      class="running-loop-wrapper">
      <!-- <span @click="changeRunLoop(false)" style="height:30px;line-height:30px;padding-right: 10px;">{{$t('loop')}}:</span> -->
      <div style="display: flex; align-items: center;padding-right: 10px;margin-left: 20px;">

        <div @click="changeRunLoop(false)" class="radio-btn" :class="{
          select: !isRunLoop,
          unselect: isRunLoop,
        }"></div>
        <template v-for="index2 in [0, 1, 2, 3, 4, 5]">
          <div class="input-per-number-style border-number"
               :class="{ 'selected-cell-bg': (runTimesSelectedIndex === 0 && index2 === 1) || (runTimesSelectedIndex === 1 && index2 === 2) || (runTimesSelectedIndex === 2 && index2 === 3) || (runTimesSelectedIndex === 3 && index2 === 4) }"
               @click="changeRunTimesIndex(index2)">
            <div class="" v-if="index2 === 0">
              <i class="el-icon-caret-left"></i>
            </div>
            <div class="" v-if="index2 === 1">
              {{ showRunTimesDotValue(0) }}
            </div>
            <div class="" v-if="index2 === 2">
              {{ showRunTimesDotValue(1) }}
            </div>
            <div class="" v-if="index2 === 3">
              {{ showRunTimesDotValue(2) }}
            </div>
            <div class="" v-if="index2 === 4">
              {{ showRunTimesDotValue(3) }}
            </div>
            <div class="" v-if="index2 === 5">
              <i class="el-icon-caret-right"></i>
            </div>
      ...
      <div style="display: flex; align-items: center;padding-right: 10px;">
        <div @click="changeRunLoop(true)" class="radio-btn" style="left:135px;" :class="{
          select: isRunLoop,
          unselect: !isRunLoop,
        }">
        </div>
        <span @click="changeRunLoop(true)" style="height:30px;line-height:30px;width: 60px;">{{$t('infinite')}}</span>
      </div>
    </div>
    <div style="position:absolute;right:5px;top:0;">  
      <button v-if="model.localTeach.isTeachRunningUUID === ''" class="start-btn" @click="onstart">
      </button>
      <button v-if="model.localTeach.isTeachRunningUUID !== ''" class="stop-btn" @click="onstop">
      </button></div>
    </div>
    <!-- <div v-if="isShowPointEdit === true">
      <button  class="add-btn" @click="onAdd.root-wrapperPoint"> </button>
      <button  class="timer-btn" @click="onAddTimer"></button>
      <button class="gripper-btn" @click="onAddGripper"> </button>
      <button v-if="model.localTeach.curSelectedIndex >= 0" class="del-btn" @click="onDelete"></button>
    </div> -->
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    onstart() {
      ...
      this.model.robot.state.local.selectPosIndex = -1;
      ...
      if (this.model.localTeach.curProj.files.length > 0) {
        ...
        window.CommandsTeachSocket.runTeach(uuid, opt, (dict) => {
          if (dict.code === 0) {
            ...
            this.model.commonStatusMgr.changeControlEditTab = false;
          }
          if (dict.code === 1111) {
    ...
    onAddGripper() {
      ...
      points.splice(index + 1, 0, [8000, pos]);
      this.save(() => {
        this.model.localTeach.onSelect(null, index + 1);
      });
    },
  ...
  computed: {
    curFilePath() {
      let path = this.model.localTeach.curSelectedTreeItem.uuid.replace('/record', '');
      path = `${path}(${this.projIndex})`
      return path.length > 32 ? `...${path.substr(-32)}` : path;
    },
    isConnected() {
...
.start-btn {
  ...
  background-size: 21px 21px;
  background-repeat: no-repeat;
  background-image: url('./../../assets/img/edit/recording/point_icon_play.svg');
  cursor: pointer;
}
.stop-btn {
  // position: absolute;
