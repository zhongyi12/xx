    ...
    <div class="recording">
      ...
      <div v-if="model.localTeach.editState===false">
        <div v-if="model.localTeach.curSelectedTreeItem.type==='proj'">
          ...
          <!-- <button v-if="model.localTeach.visible.starRecording === false" :disabled="model.localTeach.isTeachRunningUUID !==''" :class="cannotEdit" class="bottom-btn start-recording-btn" @click='startRecord()'>{{ $t('record.startRecord') }}</button>
          <button v-if="model.localTeach.visible.starRecording === true" class="bottom-btn finish-recording-btn" @click="finishRecord(model.localTeach.curEditingFileUUID)">{{ $t('record.finishRecord') }}</button> -->
          <!-- start btn -->
...
export default {
  props: ['onwinresize', 'cancelEdit', 'finishRecordOK', 'scrollTo'],
  i18n: {
    messages: {
      en: {
        times: 'Times',
