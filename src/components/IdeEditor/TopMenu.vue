<template>
  <div>
    <div class="float-left" style="margin-left: 7px;margin-top: 3px;width:30px; height:24px;" @click="listProjects()">
    ...
    <!-- <div class="el-icon-setting setting-icon float-left" title="Setting" @click="model.ideModel.showSettingDialog = true;"></div> -->
    <div class="status-icon float-left" :title="$t('status')" :class="{'el-icon-circle-check': model.ideModel.isReady && model.robot.state.info.socketConnected, 'el-icon-circle-close': !model.ideModel.isReady || !model.robot.state.info.socketConnected}" :style="{color: model.ideModel.isReady && model.robot.state.info.socketConnected ? '#52bf53' : '#e15960'}"></div>
    <span>
...
export default {
  i18n: {
    messages: {
      en: {
        ...
        stopAll: 'Stop all running scripts and stop the arm',
  ...
  computed: {
    hasRunProgram() {
      return (this.model.ideModel.consoleItems || []).some((element) => { return element.run === true });
  ...
  methods: {
    ...
    rename() {
      console.log(`model.ideModel.curProjTree.name = ${this.model.ideModel.curProjTree.name}`)
      if (this.model.ideModel.selectNode.path === '/') {
        ...
        this.model.ideModel.dialogTitle = `${this.$t('renameFolder')} (/${this.model.ideModel.curProjTree.name}${this.model.ideModel.selectNode.path})`
        ...
        this.model.ideModel.dialogTitle = `${this.$t('renameFile')} (/${this.model.ideModel.curProjTree.name}${this.model.ideModel.selectFilePath})`
        this.model.ideModel.dialogInputText = `${name}`;
      }
      this.model.ideModel.dialogTips = '';
      this.model.ideModel.showFileDialog = true;
    ...
    delFile() {
      console.log(`model.ideModel.curProjTree.name = ${this.model.ideModel.curProjTree.name}`)
      const name = path.basename(this.model.ideModel.selectNode.path);
      let del_name = this.model.ideModel.selectNode.path;
      if (this.model.ideModel.selectNode.type === 'dir') {
        del_name = path.dirname(del_name);
      }
      this.model.ideModel.dialogTitle = `${this.$t('delete')} /${this.model.ideModel.curProjTree.name}${del_name}?`;
    ...
    stopAll() {
      for (let i = 0; i < this.model.ideModel.consoleItems.length; i++) {
        if (this.model.ideModel.consoleItems[i].run === true) {
          this.model.ideModel.stopPythonProgram(this.model.ideModel.consoleItems[i].id, true);
        }
      }
