<template>
  <div>
    <el-tree
      id="tree-root"
      class="ide-project-list noselected tree-py-icon"
...
export default {
  ...
  watch: {
    ...
    'model.ideModel.selectNodePath': {
      handler(cur, old) {
        const self = this;
        setTimeout(() => {
          if (self.model.ideModel.selectNodePath !== null) {
            self.$refs.tree.setCurrentKey(self.model.ideModel.selectNodePath);
            if (self.$refs.tree.getCurrentNode() !== null) {
              self.model.ideModel.selectNode = self.$refs.tree.getCurrentNode();
            };
          }
