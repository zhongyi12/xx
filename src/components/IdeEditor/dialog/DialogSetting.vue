...
<template>
  <div id="ide-setting" class="noselected">
    <div class="dialog-wrap">
      <div class="dialog-cover" @click="closeMyself"></div>
      <div class="dialog-content">
        <span class="top-title">{{ $t('settingTitle') }}</span>
        <div style="margin-top:156px;">
          <div class="float-left btn-cancel" @click="closeMyself">
            {{ $t('cancel') }}
          </div>
          <div class="float-left btn-delete cursor-pointer" @click="onSave">
            {{ $t('save') }}
          </div>
        </div>
      </div>
...
export default {
  i18n: {
    messages: {
      en: {
        settingTitle: 'IDE Setting',
        cancel: 'Cancel',
        delete: 'Delete',
      },
      cn: {
        settingTitle: '集成开发环境设置',
        cancel: '关闭',
        save: '保存',
      },
    },
  ...
  methods: {
    closeMyself() {
      this.model.ideModel.showSettingDialog = false;
