  ...
  watch: {
    'item.content': {
      handler(curVal, oldVal) {
        ...
        let line = self.codemirror.getLine(cursor.line);
        line = line.substring(0, cursor.ch);
        let list = line.split(' ');
        list = list[list.length - 1].split('.');
        self.complete_prefix = list[list.length - 1];
        // if (cursor.ch === 0 || !self.complete_prefix || self.ignorePrefixList.includes(self.complete_prefix[self.complete_prefix.length - 1])) {
