      ...
      <div class="float-left console-tab" name="console-tab" :id="getTabId()" :style="{'background': selected? '#3F4955' : 'transparent'}">
        <div class="float-left console-tab-item-border">
          ...
          <div class="float-left console-tab-item" @click="$emit('select-item', item)">
            <div class="float-left" :class="{'console-tab-background-color-unselect': !selected, 'console-tab-background-color': selected}">{{ selected && !(model.ideModel.selectConsoleItem.name === 'Terminal' && model.ideModel.selectConsoleItem.path === 'Terminal') ? '/' + model.ideModel.curProjTree.name + item.path : (item.name === 'Terminal' && item.path === 'Terminal') ? $t('terminal') : item.name }}</div>
          </div>
