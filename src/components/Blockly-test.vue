<template>
  <div class="blockly-wrapper">
      <CommonTopMenuLeft :backTitle="$t('title')"></CommonTopMenuLeft>
    <div class="main-block-wrapper">
      ...
      <div :class="isShowProjList === false ? 'show-file-list': 'no-show-file-list'">
        <file-list
      ...
      <div><blocklySidebarLeft
        ...
        :onCancleEditBlock='onCancleEditBlock'
        :onConfirmEditBlock='onConfirmEditBlock'
        :onCancle='onCancle'></blocklySidebarLeft></div>


      <div style="position:absolute;top:54px;left:150px;z-index:2000;background:#666664;" v-show="blockly.Running && model.robot.event.GPIOEvent.show">
        <ul>
      ...
      <div
        ...
        style="position:absolute;width:100%;bottom:0px;background:white;opacity:0.5;right:70px;margin-left:auto;top:54px;z-index:40;">
      </div>
      <div
        v-if="blockly.Running"
        style="position:absolute;width:115px;bottom:0px;background:white;opacity:0.5;left:0px;top:54px;z-index:400;">
      </div>
    </div>
    ...
    <div v-if="model.robot.state.showSetControlValueDialog" class="com-side-control-wrapper" style="position:absolute;right:70px;top:54px;z-index:1000;background:rgb(216, 215, 215);padding: 1%;width:94%;height:90%;overflow:hidden;">

      <!--<div class="emulator-wrapper" style="position:absolute;width:520px;height:320px;margin-top:15px;z-index:20;margin-left:15px;">
...
import blocklySidebarLeft from './Blockly/blocklySidebarLeft';
...
//    Blockly.addEndListener(this.endCallback);
  ...
  methods: {
    ...
    onlist() {
      ...
      console.log('isShowProjList', this.isShowProjList);
    },
    stopRun() {
      const self = this;
    ...
    updateMoveJointsBlock(block) {
      const values = this.localJoints;
    ...
    updateMoveCartesianBlock(block) {
      const values = [];
    ...
    updateMoveGripperBlock(block) {
      const childBlocks = this.curPositionEditingBlock.getChildren();
      childBlocks[0].setFieldValue(this.model.robot.state.local.gripper.pos, 'NUM');
      childBlocks[1].setFieldValue(this.model.robot.state.local.gripper.speed, 'NUM');
    },
    updateMoveArcToBlock(block) {
      const values = [];
      values.push(this.model.robot.state.local.position.x);
      values.push(this.model.robot.state.local.position.y);
      values.push(this.model.robot.state.local.position.z);
      ...
      values.push(this.model.robot.state.local.orientation.yaw);
      values.push(-1);
      const fieldRow = block.getChildren()[0].inputList[0].fieldRow;
      for (let i = 0; i < values.length; i += 1) {
        fieldRow[i * 2 + 2].setText(values[i]);
      }
    },
    updateMove7Block(block) {
      const values = this.localJoints;
      const fieldRow = block.getChildren()[0].inputList[0].fieldRow;
      for (let i = 0; i < this.model.robot.state.info.xarm_axis; i += 1) {
        fieldRow[i * 2 + 2].setText(values[i]);
    ...
    updateGripperSpeedBlock(block) {
      const childBlocks = block.getChildren();
      childBlocks[0].setFieldValue(this.model.robot.state.remote.gripper.speed, 'NUM');
    },
    updateGripperPositionBlock(block) {
      const childBlocks = block.getChildren();
      childBlocks[0].setFieldValue(this.model.robot.state.remote.gripper.pos, 'NUM');
    },
    updateSetSpeedBlock(block) {
      const childBlocks = block.getChildren();
      childBlocks[0].setFieldValue(this.model.robot.state.remote.speed, 'NUM');
    },
    updateSetAccBlock(block) {
      const childBlocks = block.getChildren();
      childBlocks[0].setFieldValue(this.model.robot.state.remote.acceleration, 'NUM');
    },
    updateSetAngleSpeedBlock(block) {
      const childBlocks = block.getChildren();
      childBlocks[0].setFieldValue(this.model.robot.state.remote.angle_speed, 'NUM');
    },
    updateSetAngleAccBlock(block) {
      const childBlocks = block.getChildren();
      childBlocks[0].setFieldValue(this.model.robot.state.remote.angle_acceleration, 'NUM');
    },
    onCreateBlock(createBlock) {
      const createBlockType = createBlock.type;
      switch (createBlockType) {
        case 'move_joints':
          this.updateMoveJointsBlock(createBlock);
          break;
        case 'move_cartesian':
          this.updateMoveCartesianBlock(createBlock);
          break;
        case 'move_arc_to':
          this.updateMoveArcToBlock(createBlock);
          break;
        case 'move_7':
          this.updateMove7Block(createBlock);
          break;
        case 'gripper_set':
          this.updateGripperSetBlock(createBlock);
          break;
        case 'gripper_speed':
          this.updateGripperSpeedBlock(createBlock);
          break;
        case 'gripper_position':
          this.updateGripperPositionBlock(createBlock);
          break;
        case 'set_speed':
          this.updateSetSpeedBlock(createBlock);
          break;
        case 'set_acceleration':
          this.updateSetAccBlock(createBlock);
          break;
        case 'set_angle_speed':
          this.updateSetAngleSpeedBlock(createBlock);
          break;
        case 'set_angle_acceleration':
          this.updateSetAngleAccBlock(createBlock);
          break;
      }
      this.createFlag = 0;
      this.createBlockId = null;
    },
    clickBtnUploadBlock(btnBlock) {
      ...
      this.uploadBlock = btnBlock.parentBlock_.inputList[0].fieldRow[11];
      ele.click();
      this.isUpload = false;
    },
    clickMoveJoint(moveBlock, checkOnline) {
      const curClickJoints = [];
        const fieldRow = moveBlock.inputList[0].fieldRow;
        for (let i = 0; i < 7; i += 1) {
        ...
        if (checkOnline === false || this.stateOnline) {
    ...
    clickMoveCartesian(moveBlock, num) {
      const curCartesian = [];
      const fieldRow = moveBlock.inputList[0].fieldRow;
      for (let i = 0; i < num; i += 1) {
        curCartesian[i] = Number(fieldRow[i * 2 + 2].getText());
      }
      const wait = fieldRow[fieldRow.length - 1].getText() === 'true' ? true : false;
      if (this.stateOnline) {
        const data = {};
        data.position = {};
        data.orientation = {};
        data.position.x = curCartesian[0] || 0;
        data.position.y = curCartesian[1] || 0;
        data.position.z = curCartesian[2] || 0;
        data.orientation.roll = curCartesian[3] || 0;
        data.orientation.pitch = curCartesian[4] || 0;
        data.orientation.yaw = curCartesian[5] || 0;
        data.orientation.r = curCartesian[6] || -1;
        const self = this;
    ...
    clickSetGripper(moveBlock) {
      const childBlocks = moveBlock.getChildren();
    ...
    clickMoveCircle(moveBlock) {
      const childBlocks = moveBlock.getChildren();
      const pose1FieldRow = childBlocks[0].inputList[0].fieldRow;
      const pose1 = [0, 0, 0, 0, 0, 0];
      for (let i = 0; i < 6; i += 1) {
        pose1[i] = Number(pose1FieldRow[i * 2 + 2].getText());
      } 
      const pose2FieldRow = childBlocks[1].inputList[0].fieldRow;
      const pose2 = [0, 0, 0, 0, 0, 0];
      for (let i = 0; i < 6; i += 1) {
        pose2[i] = Number(pose2FieldRow[i * 2 + 2].getText());
      }
      const percent = Number(childBlocks[2].inputList[0].fieldRow[0].getText());
      if (this.stateOnline) {
        window.CommandsRobotSocket.moveCircle(pose1, pose2, percent, false);
      }
      else {
        this.$message({
          message: this.$t('robotIsNotOnline'),
    ...
    clickBtnMoveBlock(btnBlock, checkOnline) {
      const moveBlock = btnBlock.parentBlock_;
      const moveBlockType = moveBlock.type;
      this.curPositionEditingBlock = moveBlock;
      console.log('clickBtnMoveBlock:', moveBlockType, moveBlock)
      switch (moveBlockType) {
        case 'move_joints':
          this.clickMoveJoint(moveBlock, checkOnline);
          break;
        case 'move_cartesian':
          this.clickMoveCartesian(moveBlock, 7);
          break;
        case 'position_c':
          this.clickMoveCartesian(moveBlock, 6);
          break;
        case 'move_circle':
          this.clickMoveCircle(moveBlock);
          break;
        case 'gripper_set':
          this.clickSetGripper(moveBlock);
          break;
      }
    },
    clickBtnEditBlock(btnBlock) {
      const editBlock = btnBlock.parentBlock_;
      const editBlockType = editBlock.type;
      this.curPositionEditingBlock = editBlock;
      this.model.robot.state.showSetControlValueDialog = true;
      this.isEditBlock = true;
      this.clickBtnMoveBlock(btnBlock, false);
    },
    clickBtnBlock(btnBlock) {
      this.isMove = true;
      // console.log('debug:', event.type, block.type, element);
      const curType = btnBlock.parentBlock_.type;
      this.curPositionEditingBlock = btnBlock.parentBlock_;
      if (curType === 'position7') {
        const curClickJoints = [];
        const fieldRow = btnBlock.parentBlock_.inputList[0].fieldRow;
        for (let i = 0; i < 7; i += 1) {
          if (i < this.model.robot.state.info.xarm_axis) {
            curClickJoints[i] = Number(fieldRow[i * 2 + 2].getText());
          }
        ...
        // const wait = btnBlock.parentBlock_.childBlocks_[0].inputList[0].fieldRow[0].getText() === 'true' ? true : false;
        ...
        const curClickJoints = [];
        const childBlocks = btnBlock.parentBlock_.getChildren();
        for (let i = 0; i < 7; i += 1) {
        ...
        const curClickJoints = [];
        console.log(btnBlock.parentBlock_)
        const fieldRow = btnBlock.parentBlock_.inputList[0].fieldRow;
        for (let i = 0; i < 6; i += 1) {
          curClickJoints[i] = Number(fieldRow[i * 2 + 2].getText());
        }
    ...
    onClickBlock(block) {
      ...
      switch (blockType) {
        ...
        case 'btn_edit':
        case 'btn':
          this.onClickBtnBlock(block);
          break;
        case 'studio_copy_recording':
        case 'studio_run_app':
          this.popDialog(blockType);
      }
    },
    onClickBtnBlock(btnBlock) {
      const btnBlockType = btnBlock.type;
      switch (btnBlockType) {
        case 'btn_upload':
          this.clickBtnUploadBlock(btnBlock);
          break;
        case 'btn_move':
          this.clickBtnMoveBlock(btnBlock);
          break;
        case 'btn_edit':
          this.clickBtnEditBlock(btnBlock);
          break;
        case 'btn':
          this.clickBtnBlock(btnBlock);
          break;
      }
    },
    onUpdateBlock(block) {
      if (!this.curPositionEditingBlock) {
        return;
      }
      const editBlockType = this.curPositionEditingBlock.type;
      switch (editBlockType) {
        case 'move_joints':
          this.updateMoveJointsBlock(this.curPositionEditingBlock);
          break;
        case 'move_cartesian':
          this.updateMoveCartesianBlock(this.curPositionEditingBlock);
          break;
        case 'position_c':
          this.updateMoveCartesianBlock(this.curPositionEditingBlock);
          break;
        case 'gripper_set':
          this.updateMoveGripperBlock(this.curPositionEditingBlock);
          break;
      }
    },
    onConfirmEditBlock() {
      console.log('onConfirmEditBlock', this.curPositionEditingBlock);
      this.isEditBlock = false;
      this.model.robot.state.showSetControlValueDialog = false;
      this.onUpdateBlock();
    },
    onCancleEditBlock() {
      console.log('onCancleEditBlock');
      this.isEditBlock = false;
      this.model.robot.state.showSetControlValueDialog = false;
    },
    onChangeEvent(event) {
      ...
      if (event.type === Blockly.Events.BLOCK_MOVE && this.createFlag === 2) {
        ...
        this.onCreateBlock(createBlock);
        return;
      }
  ...
  computed: {
    ...
    notSaved() {
      console.log('block length111',this.saveStatus,`${this.pre}*`, this.blocksLength())
  ...
  watch: {
    curPositionEditingBlock(newValue) {
      if (newValue) {
        ...
        newValue.setColour(BlocklyDefaultColor.COLOR_SETTING);
